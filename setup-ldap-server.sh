...
printf "${BLUE}-*- Configuring LDAP Server...\n${NC}"
...
rootdn  "cn=ambari,dc=apache,dc=org"
...
dn: ou=dev,dc=apache,dc=org
objectClass: top
objectClass: OrganizationalUnit
ou: dev

dn: ou=people,ou=dev,dc=apache,dc=org
objectClass: top
objectClass: OrganizationalUnit
ou: people

dn: ou=groups,ou=dev,dc=apache,dc=org
objectClass: top
objectClass: OrganizationalUnit
ou: groups
EOFDELIM

ldapadd -x -D "cn=ambari,dc=apache,dc=org" -w yoursecretpassword -f ldap-init.ldif

cp $CWD/users_groups.ldif /etc/openldap/users_groups.ldif

ldapadd -x -D "cn=ambari,dc=apache,dc=org" -w yoursecretpassword  -f users_groups.ldif
